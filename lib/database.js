...
export async function getUserAlbums(id)
{
  ...
  try 
  {
    // Connection to server
    const db = await connectToDatabase();
    let albums = await db.collection("albums").findOne({ userId: new ObjectId(id) });
    if (!albums) {
    return {
      status: 404,
...
{
  console.log("Album_Creation->user id-.>",userid.userId);
  try 
  {
    // Connection to server
    const db = await connectToDatabase();
    let albums = await db.collection("albums").insertOne({ userId: new ObjectId(userid.userId),
                                                           name : userid.name
     });
     console.log(albums);
    if (!albums) {
