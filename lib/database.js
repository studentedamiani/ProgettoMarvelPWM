...
import { MongoClient, ServerApiVersion,ObjectId,Double } from 'mongodb';
...
export async function register_user(res,user) {
  try {
    // Connection to server
    user.credits= Double(user.credits);
    const db = await connectToDatabase();
...
export async function variate_credits(credits)
{
  ...
  console.log(Double(user.credits));
  Double(credits.newCredits) = Double(user.credits) + Double(credits.credits);
  ...
  try {
    const result = await db.collection("users")
      .updateOne(
        ...
        { $set: { credits: Double(credits.newCredits) } }
      );
      let NewCredit = await db.collection("users").findOne({ username: credits.username });
      return {
        status: 200,
        result,
        credits: NewCredit.credits
